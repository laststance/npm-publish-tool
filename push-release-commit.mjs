...
/**
 * Increments a semver version string based on the specified type.
 * @param {string} version - Current version in MAJOR.MINOR.PATCH format
 * @param {'major' | 'minor' | 'patch'} type - Type of version increment
 * @returns {string} New version string
 * @throws {Error} If version format is invalid or type is not recognized
 */
...
/**
 * Creates version selection choices for the interactive prompt.
 * @param {string} currentVersion - Current version string
 * @returns {Array} Array of choice objects for inquirer select
 */
export function createVersionChoices(currentVersion) {
...
const isMainModule =
  ...
  process.argv[1]?.endsWith('push-release-commit.mjs') ||
  process.argv[1]?.endsWith('push-release-commit')
...
if (isMainModule) {
  try {
    ...
    const versionType = await select({
      ...
      choices: createVersionChoices(currentVersion),
    })
