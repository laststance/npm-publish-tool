...
import { detectPackageManager, installPackage, getPackageManagerInfo } from './lib/packageManager.mjs';
import { copyTemplate, addPackageScript, createScriptsDirectory, fileExists, ensureDirectory } from './lib/fileOperations.mjs';
import { 
  logSuccess, 
  logError, 
  logWarning, 
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const installSuccess = installPackage('release-it', projectPath, { isDev: true });
      if (!installSuccess) {
      ...
      const releaseItSuccess = copyTemplate('.release-it.json', '.release-it.json', projectPath);
      if (!releaseItSuccess) {
      ...
      const workflowSuccess = copyTemplate('release.yml', '.github/workflows/release.yml', projectPath);
      if (!workflowSuccess) {
